...
Node newNode(int data){
   ...
   N->data = data;
   N->next = NULL;
   N->prev = NULL; // idk what im supposed to do here, do I set it to last node in list or what
   return(N);
}
...
List copyList(List L) {
   ...
   if( L==NULL ){
      printf("List Error: copyList on NULL List reference\n");
      exit(EXIT_FAILURE);
   }
   if(isEmpty(L)) {
      return copiedList;
   ...
   int x = L->index;
   //append all list elements onto new list
   moveFront(L);
   for(int i = 0; i < length(L); i++) {
      append(copiedList,get(L));
      moveNext(L);
   }
   //return L to original state
   moveFront(L);
   for(int i = 0; i < x; i++) {
      moveNext(L);
   }
